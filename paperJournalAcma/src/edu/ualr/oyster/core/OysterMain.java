...
import edu.ualr.oyster.io.OysterRDFReader;
...
//                runScript.getLinkOutputPasswd()));
    ...
    private void preloadIndex() throws Exception{
        ...
        for (Iterator<String> it = getRunScript().getSources().iterator(); it.hasNext();) {
            ...
            else if (source.getSourceType().equalsIgnoreCase("RDFStore")){
            	OysterRDFReader osr = new OysterRDFReader(source.getSourcePath(), source.getEndPoint(), source.getPrefix(), source.getReferenceItems(), logger);
            	osr.open();
